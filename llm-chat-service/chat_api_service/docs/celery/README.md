...
4. [Взаимодействие с RabbitMQ](#взаимодействия-с-rabbitmq);
5. [Публикация сообщения в RabbitMQ](#публикация-события-в-rabbitmq).
...
На изображении с интерфейсом RabbitMQ показано, что брокер действительно участвует в системе. В интерфейсе видны 
активные очереди Celery, сообщения в них и подключения от Bot Service и Celery-воркера. Это подтверждает, что задачи 
LLM-запросов публикуются в очередь и обрабатываются асинхронно, а не исполняются синхронно в процессе бота.

![rabbitmq.png](rabbitmq.png)

![queue_messages.png](queue_messages.png)
...
|-----------------|------|-----------------------------------------------------------------|
| message_id      | UUID | Идентификатор обработанного сообщения.                          |
| answer_id       | UUID | Идентификатор сообщения ассистента, сохраненного в PostgreSQL.  |
