...
from .deps import AuthUsercaseDependency, AUTH_HEADERS, UserIdDependency
...
async def register(
        ...
        auth_usercase: AuthUsercaseDependency
):
    """
    Регистрация нового пользователя.

    :param user_data: Данные для регистрации.
    :param auth_usercase: Usercase для работы с аутентификацией.

    :return: Данные созданного пользователя.
    """
    try:
        user: UserPublic = await auth_usercase.register(
...
async def login(
        ...
        auth_usecase: AuthUsercaseDependency
):
    """
    Вход в систему (OAuth2 совместимый).

    :param data: Данные для входа.
    :param auth_usecase: Usercase для работы с аутентификацией.
...
async def get_my_profile(
    ...
    """
    try:
        user = await auth_usercase.get_profile(user_id)
